...
class ActiveRecord::Base
      ...
      self.class.reflect_on_all_attachments
        .each { |association|
          ...
          Array(send(a_name.to_sym)).each_with_index { |attachment, index|
            ...
            blob_path = Rails.root.join("test/fixtures/files/#{blob.filename}").to_s
  ...
  def dump_raw_fixture(text)
    fixture_file = Rails.root.join("test/fixtures/#{self.class.name.underscore.pluralize}.yml").to_s
